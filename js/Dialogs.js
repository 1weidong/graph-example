...
console.log(value)
	...
	var addTextArea = function(index, name, value)
	{
		...
		switch (name) {
			// case 'type':
			// 	let typeData = ['module', 'string', 'boolean', 'integer']
			// 	texts[index] = form.addCombo(name, false, 1);
			// 	for (let i=0; i < typeData.length; i++) {
			// 		if(value == typeData[i]) {
			// 			form.addOption(texts[index], value, value, true);
			// 		} else {
			// 			form.addOption(texts[index], typeData[i], typeData[i], false);
			// 		}
			// 	}
			// 	break;
			default:
 ...
 * Constructs a new metadata dialog.
 */
var AddElementDialog = function(ui, cell)
{
	var div = document.createElement('div');
	var graph = ui.editor.graph;
	var value = graph.getModel().getValue(cell);
	var cellValue = value.tagName;
	var sequenceList = ['sequenceItem'];
	var enumList = ['EnumItem'];
	...
	function transformXml(options, name) {
		...
		// var filed = doc.createElement('filed')
		// obj.appendChild(filed)
		if (options.length > 0) {
			optionsArr = options;
			for(var i=0; i<optionsArr.length; i++) {
				obj.setAttribute(optionsArr[i].name, optionsArr[i].value);
				// filed.setAttribute(optionsArr[i].name, optionsArr[i].value);
			}
		} else {
