...
Format.prototype.refresh = function()
{
	...
	var cellValue = graph.getSelectionCell();
	if (cellValue) {
	...
	else
	{
		...
		else
		{
			// propertyPanle
			label.style.borderLeftWidth = '0px';
			mxUtils.write(label, mxResources.get('propertyPanle'));
			div.appendChild(label);
			var propertyPanel = div.cloneNode(false);
			propertyPanel.style.display = 'none';
			this.panels.push(new PropertyPanel(this, ui, propertyPanel));
			this.container.appendChild(propertyPanel);
			addClickHandler(label, propertyPanel, idx++);
		}
...
ElementPanel = function(format, editorUi, container) {
	BaseFormatPanel.call(this, format, editorUi, container);
	this.init();
}
mxUtils.extend(ElementPanel, BaseFormatPanel);

ElementPanel.prototype.init = function() {
	this.container.appendChild(this.showelement(this.createPanel()));
}

/*
* 子元素显示
* */
ElementPanel.prototype.showelement = function (container) {
	var div = document.createElement('div');
	var ui = this.editorUi;
	var graph = ui.editor.graph;
	...
	for(var i=0; i<list.length; i++) {
		if (i == 0) {
			div.innerHTML += '<ul class="add-element"><li class="elment-item"><input class="elment-item-select" type="checkbox" name="'+cellValue+'" id="select'+i+'" value="'+list[i]+'" /><label for="select'+i+'">' + list[i] + '</label></li></ul>';
		} else{
			div.childNodes[0].innerHTML += '<li class="elment-item"><input class="elment-item-select" type="checkbox" name="'+cellValue+'" id="select'+i+'" value="'+list[i]+'" /><label for="select'+i+'">' + list[i] + '</label></li>';
		}
	}
	...
	var applyBtn = mxUtils.button(mxResources.get('apply'), ()=>
	{
		try
		{
			var checkboxs = document.getElementsByName(cellValue);
			var ck = new Array();
			var j = 0;
			...
			if (j == 0) {
				...
				try
				{
					...
					for (var i=0; i<ck.length; i++) {
						var field = new mxCell(transformXml(fieldParams(ck[i]), ck[i]), new mxGeometry(0, 0, 100, 26), 'text;strokeColor=none;fillColor=none;align=left;verticalAlign=top;spacingLeft=4;spacingRight=4;overflow=hidden;rotatable=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;');
						field.vertex = true;
...
PropertyPanel.prototype.init = function() {
	this.container.appendChild(this.showProperty(this.createPanel()));
}

/*
* 属性显示
* */
PropertyPanel.prototype.showProperty = function (container) {
	var div = document.createElement('div');
	var ui = this.editorUi;
	var graph = ui.editor.graph;
	var cell =  graph.getSelectionCell();
...
ArrangePanel.prototype.addGroupOps = function(div)
{
	...
	if (graph.getSelectionCount() == 1)
	{
		...
		if (cell.value.tagName !== 'module' && cell.value.tagName !== 'sequence') {
			btn = mxUtils.button(mxResources.get('addElement'), mxUtils.bind(this, function(evt)
			{
				this.editorUi.actions.get('addElement').funct();
			}));

			btn.setAttribute('title', mxResources.get('addElement') + ' (' + this.editorUi.actions.get('addElement').shortcut + ')');
			btn.style.width = '100px';
			btn.style.marginBottom = '2px';
			div.appendChild(btn);
			count++;
